<p align="left">
  <img src="https://komarev.com/ghpvc/?username=Curiousminded06&label=Profile%20views&color=0e75b6&style=for-the-badge" alt="profile views" />
</p>

# I'm Sahil Dale

AI & Data Science Student at DAVV | AI Engineer | Backend Developer | LLM & Scalable AI Systems Enthusiast

---

<!--
### 🌐 Portfolio

Visit my portfolio: [Coming Soon]()

---
-->
### 👋 About Me

- Passionate about building intelligent AI-powered applications and scalable backend systems.
- Currently exploring LLM Engineering, RAG pipelines, Multimodal AI, and distributed systems.
- Interested in AI Engineering, Backend Development, DevOps, and System Design.
- I enjoy transforming complex real-world problems into efficient software solutions.

---

<!--
## 🏆 Badges

Add your Holopin or other badges here.

---
-->
## 🔧 Tech Stack

### 🤖 AI & Machine Learning
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&style=flat)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?logo=pytorch&style=flat)
![Scikit-Learn](https://img.shields.io/badge/-Scikit--Learn-F7931E?logo=scikit-learn&style=flat)
![Machine Learning](https://img.shields.io/badge/-Machine%20Learning-102A43?style=flat)
![LLMs](https://img.shields.io/badge/-LLMs-6A1B9A?style=flat)
![RAG](https://img.shields.io/badge/-RAG-0F9D58?style=flat)

### 💻 Frontend
![React](https://img.shields.io/badge/-React-black?logo=react&style=flat)
![Next.js](https://img.shields.io/badge/-Next.js-black?logo=next.js&style=flat)
![JavaScript](https://img.shields.io/badge/-JavaScript-black?logo=javascript&style=flat)
![TypeScript](https://img.shields.io/badge/-TypeScript-black?logo=typescript&style=flat)

### ⚙️ Backend
![Node.js](https://img.shields.io/badge/-Node.js-black?logo=node.js&style=flat)
![Spring Boot](https://img.shields.io/badge/-Spring%20Boot-6DB33F?logo=springboot&style=flat)
![REST API](https://img.shields.io/badge/-REST%20API-blue?style=flat)

### 🗄️ Databases
![MongoDB](https://img.shields.io/badge/-MongoDB-black?logo=mongodb&style=flat)
![MySQL](https://img.shields.io/badge/-MySQL-black?logo=mysql&style=flat)
![Vector DB](https://img.shields.io/badge/-Vector%20Database-512BD4?style=flat)
![Qdrant](https://img.shields.io/badge/-Qdrant-DC382D?style=flat)

### 🚀 DevOps & Tools
![Git](https://img.shields.io/badge/-Git-black?logo=git&style=flat)
![GitHub](https://img.shields.io/badge/-GitHub-181717?logo=github&style=flat)
![Docker](https://img.shields.io/badge/-Docker-black?logo=docker&style=flat)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-black?logo=kubernetes&style=flat)
![GitHub Actions](https://img.shields.io/badge/-GitHub%20Actions-2088FF?logo=github-actions&style=flat)

### 📚 Core Concepts
![SOLID](https://img.shields.io/badge/-SOLID%20Principles-0A66C2?style=flat)
![System Design](https://img.shields.io/badge/-System%20Design-FF9800?style=flat)
![Data Structures](https://img.shields.io/badge/-Data%20Structures-4CAF50?style=flat)

---

<!--
## 📈 GitHub Stats

![Sahil's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight)

---
-->

## 🗂️ Featured Projects

### 🚗 Vehicle Damage Classification
CNN-powered image classification system capable of detecting six categories of vehicle damage.

**Tech Stack:** Python • PyTorch • CNN • Streamlit

---

### 💳 Credit Risk Prediction System
End-to-end machine learning pipeline for loan default prediction using feature engineering, SMOTE-Tomek balancing, and Optuna hyperparameter optimization.

**Highlights**
- AUC ≈ **0.98**
- F1 Score ≈ **0.80–0.85**

**Tech Stack:** Python • Scikit-Learn • XGBoost • Optuna

---

### 📄 RAG-based PDF Question Answering
Built a Retrieval-Augmented Generation pipeline that indexes PDFs into Qdrant and answers contextual questions using Gemini.

**Tech Stack:** Node.js • Gemini API • Qdrant • RAG

---

## 🌱 Currently Learning

- AI Engineering
- Advanced LLM Systems
- Agentic AI
- Multimodal AI
- Distributed Backend Systems
- Kubernetes & Cloud Native Applications

---

## 🏆 Leadership & Community

- 🎯 Organized **International Conference iBCD-2024** and led volunteer teams.
- 👨‍💻 Co-Lead of **Datashores Tech Community**.
- 🚀 Team Lead for **Smart India Hackathon 2024** project on *Direct Market Access for Farmers*.

---

## 📫 Connect With Me

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?logo=linkedin&style=flat)](https://www.linkedin.com/in/sahil-dale-96a984287)

📧 **sahildaleworkspace@gmail.com**

---

> *"Building scalable AI systems that solve real-world problems, one commit at a time."* 🚀
